...
export default function CountriesPage() {
  ...
  const featuredCountry = countries.reduce((featured, country) => {
    const featuredCount =
      featured.storySlugs.length + featured.recipeSlugs.length;
    const currentCount = country.storySlugs.length + country.recipeSlugs.length;

    return currentCount > featuredCount ? country : featured;
  }, countries[0]!);
          ...
          <aside className="rounded-[2rem] border border-[#ddd2c5] bg-white/85 p-7 shadow-[0_18px_50px_rgba(80,60,35,0.06)]">
            ...
            <dl className="mt-7 grid grid-cols-3 gap-3">
              ...
              <div className="rounded-2xl bg-[#f7f3ed] p-4 text-center">
                ...
                <dd className="mt-2 text-2xl font-black">{totalRecipes}</dd>
              </div>
            </dl>
          </aside>
        </div>
      ...
      >
        <div className="mb-12 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          ...
          <p className="max-w-md break-keep text-sm leading-7 text-neutral-600">
            각 카드를 선택하면 대표적인 맛과 재료, 지역·공간별 차이, 문화
            이야기와 레시피를 함께 볼 수 있습니다.
          </p>
        </div>
