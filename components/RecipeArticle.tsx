    ...
    <div className="recipe-editorial">
      <article>
          ...
          <div className="editorial-wrap">
            ...
            <div className="recipe-cover-grid">
              <div className="recipe-cover-copy">
                ...
                <p className="recipe-screen-hint">식문화부터 조리법까지, 한 장씩 크게 만나보세요.</p>
              </div>
        ...
        <section id="culture" className="recipe-culture editorial-section">
          <div className="editorial-wrap editorial-section-grid"><div><p className="editorial-eyebrow">01 / FOOD & CULTURE</p><h2>맛을 알면,<br />이야기가 보입니다.</h2><p className="section-caption">한 접시에 담긴 배경을 먼저 만나보세요.</p></div><div><p className="culture-prose">{recipe.culturalNote}</p><div className="culture-links">{country && <Link href={`/countries/${country.slug}`}>{country.nameKo}의 식탁 ↗</Link>}{stories[0] && <Link href={`/stories/${stories[0].slug}`}>문화 이야기 읽기 ↗</Link>}</div></div></div>
        </section>
        ...
        <section id="sources" className="editorial-section recipe-sources"><div className="editorial-wrap editorial-section-grid"><div><p className="editorial-eyebrow">05 / FURTHER READING</p><h2>이야기의 출처</h2><p className="section-caption">문화와 조리 정보를 확인한 자료입니다.<br />등록일 {recipe.publishedAt}</p></div><ol>{recipe.sources.map((source, index) => <li key={source.url}><span>{String(index + 1).padStart(2, "0")}</span><a href={source.url} target="_blank" rel="noreferrer">{source.label}<span aria-hidden="true">↗</span></a></li>)}</ol></div></section>
      </article>
